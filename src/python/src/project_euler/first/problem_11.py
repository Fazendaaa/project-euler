...
def each_cons(vector: list[int], limit: int) -> list[list[int]]:
    ...
    Args:
        vector (list[int]): Input list of integers
        limit (int): Size of each consecutive group

    Returns:
        list[list[int]]: List of lists containing consecutive elements of size limit
    """
...
def greatest_product_vector(vector: list[int], limit: int) -> int:
    ...
    Args:
        vector (list[int]): Input list of integers
        limit (int): Number of consecutive elements to multiply

    Returns:
        int: Maximum product found, or 0 if no products possible
    """
    results: list[int] = []

    for cons in each_cons(vector, limit):
        results.append(reduce(mul, cons, 1))
...
def greatest_product_matrix(matrix: list[list[int]], limit: int) -> int:
    ...
    Returns:
        int: Maximum product found across all rows
    """
    results: list[int] = []
...
def greatest_product_diagonals(matrix: list[list[int]], limit: int) -> int:
    ...
    Returns:
        int: Maximum product found across all diagonals
    """
    matrix = array(matrix)
    diags = [
        matrix[::-1, :].diagonal(i)
...
def greatest_product(matrix: list[list[int]], adjacent: int) -> int:
    ...
    Args:
        matrix (list[list[int]]): 2D list of integers
        adjacent (int): Number of adjacent elements to multiply

    Returns:
        int: Maximum product found across rows, columns and diagonals
    """
